...
export default ({
  ...
  $(LifeCycleTypes.ON_FIELD_VALUE_CHANGE, xComponentPath).subscribe(
    ({ value }) => {
      ...
      const {
        type,
        extensionSchema = { type: 'objetc', properties: {} }
      } = xComponents.find(item => item.value === value)
