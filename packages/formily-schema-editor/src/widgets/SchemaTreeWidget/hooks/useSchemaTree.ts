...
export const useSchemaTree = (props: ISchemaTreeWidgetProps) => {
  ...
  const traversalSchema = useMemo(() => {
    return function _traversal(
      ...
      return {
        ...
        children:
          schema.type === 'array'
            ? ((schema.items || ({} as any)).children || []).map(
              (child, idx) => {
                return _traversal(
                  child,
                  path.concat('0', child.key),
                  idx,
                  child.key
                )
              }
  ...
  const [activedNode, setActivedNode] = useState({
    activedNodePath: '',
    nodeType: 'object'
  })
  ...
  useEffect(() => {
    ...
    setActivedNode({
      activedNodePath: currentPath,
      nodeType: props.currentSchema.type
    })
  ...
  const setNodeFocus = (path = '', nodeType = 'object') => {
    // path、nodeType用来描述节点类型，给cursor消费
  ...
  const onMoveNode = node => {
    setTimeout(() => {
       ...
       */
      const {
        data: { path: sourcePath = [], key }
      } = draggingNode.current // 此时可以拿到node维护的 source path, name 用来生成新Path
      if (!(parentChildren.length - 1)) {
        ...
        // 当父节点有孩子时，要判断孩子的是插入到第一个还是最后一个还是中间
  ...
  const schemaUpdate = nodeElement => params => {
    ...
    const parent =
      nodeElement.parentNode.type === 'array'
        ? [...sourcePath, '0']
        : [...sourcePath]
    // debugger;
    ...
    if (nodeType === 'leaf') {
      ...
      if (!(children.length - 1)) {
        // 当前父节点没有孩子，调用onAppend
        onAppend(pathParser(parent), { ...params })
      } else {
        // 如果父节点有孩子，调用onAppendBefore
  ...
  const onDrop = params => {
    ...
    if (nextParent.data.status === 'init') {
      // 新节点不能当父亲
      return false
    }
