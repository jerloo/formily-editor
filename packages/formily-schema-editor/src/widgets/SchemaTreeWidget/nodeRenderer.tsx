...
const NodeRenderer: React.FC<INodeRendererProps> = React.memo(
  props => {
    const {
    ...
    const renderNormal = () => {
      return (
        ...
        >
          ...
          {!isRoot && <div className="rst__rowName">{node.data.title}</div>}
          {!isRoot && (
    ...
    const renderForm = () => {
      return (
        <div className="rst__formWrapper">
          <div className="rst__formWrapper__title">
            {nodeType === 'leaf' ? '添加子节点' : '添加兄弟节点'}
          </div>
          <Form
            onCancel={onCancel}
            onUpdate={onUpdate}
