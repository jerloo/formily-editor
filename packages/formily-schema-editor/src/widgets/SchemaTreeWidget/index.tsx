          ...
          return {
            title: (
              <Fragment>
                {key !== '@@root' && (
                  <i
                    className={cls('node-icon', `iconfont icon-${data.icon}`)}
                  />
                )}
                <span className="card-title">
                  {key !== '@@root' ? node.data.key : '表单'}
                </span>
              </Fragment>
            ),
            onClick: () => {
            ...
            buttons: [
              <i
                ...
                className="next-icon next-icon-copy"
                onClick={e => {
                  e.stopPropagation()
                  copyPath(copy, path)
                }}
              ...
              <i
                style={{ fontSize: '14px' }}
                className="next-icon next-icon-ashbin"
                onClick={schemaDelete(data)}
              />
            ],
