...
const SimpleArrayTable = ({
  ...
  const renderColumns = (items, index) => {
    return items.mapProperties((props, key) => {
      ...
      return (
        <FormItemDeepProvider labelCol={undefined} wrapperCol={undefined}>
  ...
  return (
    <div className="simple-array-table">
      ...
      >
          ...
          {value.map((item, index) => {
            ...
            return (
              <div className="row" key={index}>
                <div id={`row-${index}`} className="row-index">
                  {index + 1}
                </div>
                {isArr(schema.items)
        ...
        {canRemoveAll && (
          ...
          >
            <Button className="add-btn" size="large" text type="primary">
              添加
            </Button>
          </div>
        )}
